...
4. Automation сервис публикует в указанное время
...
### 4. Автоматизация публикаций

```
Automation Service (работает каждую минуту)
  ├── Получает посты со статусом SCHEDULED
  ├── Проверяет, пришло ли время публикации
  ├── Для каждого поста:
  │   ├── Получает credentials канала
  │   ├── Публикует через API социальной сети
  │   ├── Обновляет статус на PUBLISHED
  │   └── При ошибке: статус FAILED, повтор через 5 минут
