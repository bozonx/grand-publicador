...
Все запросы к этому API защищены с использованием пользовательских API токенов.
...
- **Заголовок (Header):** `x-api-key: <ВАШ_API_ТОКЕН>`
- **Альтернативный заголовок:** `Authorization: Bearer <ВАШ_API_ТОКЕН>`
